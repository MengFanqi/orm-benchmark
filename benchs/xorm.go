...
func init() {
	...
	st.InitF = func() {
		...
		st.AddBenchmark("Read", 2000*ORM_MULTI, XormRead)
...
func XormUpdate(b *B) {
	...
	for i := 0; i < b.N; i++ {
		m.Id = i
		m.Age = i
		if _, err := xo.Id(i).Update(m); err != nil {
			fmt.Println(err)
			b.FailNow()
		}
	}
...
func XormRead(b *B) {
	...
	for i := 1; i <= b.N; i++ {
		var model []Model
		if err := xo.Where("id = ?", i).Limit(1).Find(&model); err != nil {
			fmt.Println(err)
			b.FailNow()
		}
	}
...
func XormReadSlice(b *B) {
	...
	for i := 1; i <= b.N; i++ {
		var models []*Model
		if err := xo.Where("id >= ?", 1).Limit(100).Find(&models); err != nil {
			fmt.Println(err)
			b.FailNow()
		}
	}
